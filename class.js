...
function AddPrototype(target, context) {
    target.prototype.schoolYear = new Date().getFullYear();
}
function AddSchool(target, context) {
...
let Student = (() => {
    let _classDecorators = [Logger, AddPrototype, AddSchool];
...
////////////
function AddEngineering(target, context) {
    return class extends target {
        constructor(...args) {
            super(...args);
        }
...
let Employee = (() => {
    let _classDecorators = [AddEngineering];
    let _classDescriptor;
    let _classExtraInitializers = [];
    let _classThis;
    var Employee = class {
